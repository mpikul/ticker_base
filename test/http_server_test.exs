defmodule HttpServerTest do
  ...
  alias TickerBase.{Database, HttpServer, Tick}
  ...
  setup_all do
    ...
    ExMeck.expect(Database, :insert_tick!,                 fn _ -> true end)
    ExMeck.expect(Database, :get_all_ticks,                fn _ -> [] end)
    ExMeck.expect(Database, :get_ticks_from_time_range,    fn _, _, _ -> [] end)
    ExMeck.expect(Database, :get_ticks_from_current_month, fn _ -> [] end)

    ExMeck.new(Raxx)

    ExMeck.expect(Raxx, :fetch_query,
      fn %{query: :time_from_to}  -> {:ok, %{"timestamp_from" => @timestamp_from, "timestamp_to" => @timestamp_to}}
  ...
  test "HttpServer handle correct GET - all ticks" do
    ...
    assert(ExMeck.contains?(Database, {:_, {Database, :get_all_ticks, [:EURUSD]}, :_}))
  end

  test "HttpServer handle correct GET - timestamp from to" do
    HttpServer.handle_request(%Raxx.Request{method: :GET,
                                            path: ["api", "ticks", "EURUSD"],
                                            query: :time_from_to}, @aliases_map)
    assert(ExMeck.contains?(Database, {:_, {Database,
                                            :get_ticks_from_time_range,
                                            [:EURUSD,
                                              String.to_integer(@timestamp_from),
                                              String.to_integer(@timestamp_to)]}, :_}))
  end
  ...
  test "HttpServer handle correct GET - candles" do
    ...
    assert(ExMeck.contains?(Database, {:_, {Database, :get_ticks_from_current_month, [:EURUSD]}, :_}))
  end
  ...
  test "HttpServer handle incorrect GET - wrong path" do
    ...
    refute(ExMeck.contains?(Database, {:_, {Database, :get_all_ticks, [:_]}, :_}))
  end
  ...
  test "HttpServer handle correct POST - insert data" do
    ...
    tick = %Tick{symbol: :EURUSD,
                 price: String.to_float(@price),
                 timestamp: String.to_integer(@timestamp_from)}
    assert(ExMeck.contains?(Database, {:_, {Database, :insert_tick!, [tick]}, :_}))
              end
  ...
  test "HttpServer handle incorrect POST - wrong path" do
    ...
    refute(ExMeck.contains?(Database, {:_, {Database, :insert_tick!, [:_]}, :_}))
  end
